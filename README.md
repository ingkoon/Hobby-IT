...
- Redis 저장소를 활용해 메모리 상에서 빠르게 토큰을 검사하여 회원 정보를 조회

### 2) 모임 구성원 간의 화상 통화와 채팅 기능 - WebRTC
- WebRTC 기술을 사용하여, 웹 상에서 빠른 속도로 화상 통화와 채팅이 가능
- 개인 카메라 및 오디오 설정
- 화상 통화 도중에도 캔버스 페인팅 가능
- 화상 통화 배경 음악 및 배경 이미지, 채팅 컬러의 커스터마이징 기능

### 3) 모임 관리자의 가입 신청 회원 및 모임 활동 관리
- 모임을 처음 생성한 사람은 해당 모임의 관리자가 됨
- 모임 관리자는 새로 가입 신청한 회원들을 관리

### 4) 모임 게시판을 통한 모임 커뮤니티 활성화
- 최대 10장의 사진과 최대 512 글자의 내용을 작성할 수 있는 모임 게시판
- '무한 스크롤'과 '다이나믹 스크롤'을 통한 UX 제공

### 5) 모임 방명록을 통한 캔버스 페인팅
- 하루에 한 번 캔버스 다이어리 작성
- 하루 전 날짜의 작성일을 가진 방명록들만 조회 가능
- 익일부터 조회되는 이유는 사용자의 흥미를 유발하기 위함

### 6) MZ스러운 디자인과 친절한 이용가이드
