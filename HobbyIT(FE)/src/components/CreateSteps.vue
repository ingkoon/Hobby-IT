<template>
  <v-card
    class="bg mx-auto"
    style="height:500px; background-color:#0E0F28; color: white;"
    width="600"
  >
    <!-- 숫자 -->
        <v-card-title class="justify-space-between" style="display: flex; align-items: center; text-shadow: 0 4px 4px rgba(0, 0, 0, 0.25);">
          <v-avatar
            id="s1"
            size="52"
            style="border-width: 2px; border-style: solid; border-color: white; color: white; margin-left:10px;"
          ><v-icon icon="mdi-numeric-1" size="36"></v-icon></v-avatar>
          <hr style="color:white; width:150px;">
          <v-avatar
            id="s2"
            size="52"
            style="border-width: 2px; border-style: solid; border-color: white; color: white;"
          ><v-icon icon="mdi-numeric-2" size="36"></v-icon></v-avatar>
          <hr style="color:white; width:150px;">
          <v-avatar
            id="s3"
            size="52"
            style="border-width: 2px; border-style: solid; border-color: white; color: white; margin-right: 20px;"
          ><v-icon icon="mdi-numeric-3" size="36"></v-icon></v-avatar>
        </v-card-title>
    ...
    <v-window v-model="step">
      ...
      <v-window-item :value="1">
        ...
        <!-- 양식 -->
        <v-card-text>
          <h2 style="font-size:24px; text-align:center; margin: 5px;">먼저 모임의 썸네일, 이름, 카테고리를 정해볼까요?</h2>
          ...
          style="border:2px solid #24B1D0; border-radius:10px; height:40px; width:500px; padding-left:20px; margin:0 0 7px 20px;
          ...
          <div style="display: flex; align-items: center; text-align: center;">
            <v-item-group v-model="myValue" mandatory="force" selected-class="bg-pink">
              ...
              >
                <v-btn
                  :class="selectedClass"
                  style="width:70px; height:45px; justify-content:center; text-align: center; margin: 10px; border: 2px solid #FA8EB6;
                  border-radius: 50px; background-color: #0E0F28; color: white; font-size: 13px; font-weight: 700; text-shadow: 0 4px 4px rgba(0, 0, 0, 0.25);"
                  @click="toggle"
                >
                  #{{ tags[n-1] }}
                </v-btn>
      ...
      <v-window-item :value="2">
        ...
        <v-card-text>
          ...
          <div style="display: flex; align-items:center; margin: 20px 0 10px 0;">
            <v-icon color="blue-lighten-2" icon="mdi-message-question-outline" size="36" style="align-self: center;"></v-icon>
            <span style="font-size:20px; font-weight: 700; margin-left: 10px;">HOBBY 소개</span>
          </div>
          <v-textarea label="어떤 모임인가요" variant="outlined"></v-textarea>
        </v-card-text>
      </v-window-item>
      ...
      <v-window-item :value="3">
        ...
        <v-card-text>
          ...
          <div style="display: flex; align-items:center; margin: 40px 0 0 0;">
            <v-icon color="blue-lighten-2" icon="mdi-account-multiple-outline" size="30" style="align-self: center;"></v-icon>
          ...
          <v-item-group v-model="myValue" mandatory="force" selected-class="bg-pink" style="display:flex;">
              <v-item
                v-for="n in 5"
                :key="n"
                v-slot="{ selectedClass, toggle }"
              ...
              <div style="display:flex; flex-direction: column; align-items:center; margin: 0 30px 0 -2px;">
                <v-btn
                  :class="selectedClass"
                  size="60"
                  style="justify-content:center; text-align: center; margin: 15px; border: 2px solid white;
                  border-radius: 50px; background-color: #0E0F28; color: white; font-size: 20px; font-weight: 700; text-shadow: 0 4px 4px rgba(0, 0, 0, 0.25);"
                  @click="toggle"
                >
                  {{ people[n-1] }}
                </v-btn>
                <div style="width:55px; height:20px; border:2px solid white; border-radius: 40px 40px 0 0 / 17px 17px 0 0; margin-top:-10px;"></div>
              </div>
              </v-item>
            </v-item-group>
          ...
          <v-item-group mandatory="force" style="margin-left: 35px;">
            <v-container>
              <v-row>
                <v-item v-slot="{ isSelected, toggle }">
                  <v-card
                    ...
                    height="100"
                    style="background-color: #0E0F28; border: solid #24B1D0; border-width: 2px 1px 2px 2px; border-radius: 10px 0 0 10px;"
                    width="250"
                    @click="toggle"
                  >
                    <div
                      ...
                      style="color: white; font-size: 24px; font-weight: 700; display: flex; flex-direction: column; align-items: center;"
                    >
                    ...
                    >
                    자유 가입<br><v-divider style="margin:5px;"></v-divider><span style="font-size:12px; font-weight: 400;">관리자의 별도 승인 없이<br>바로 가입 및 활동이 가능합니다.</span>
                    </div>
                  </v-card>
                </v-item>
                <v-item v-slot="{ isSelected, toggle }">
                  <v-card
                    ...
                    dark
                    height="100"
                    style="background-color: #0E0F28; border: solid #24B1D0; border-width: 2px 2px 2px 1px; border-radius: 0 10px 10px 0;"
                    width="250"
                    @click="toggle"
                  >
                    <div
                    ...
                    <div
                      ...
                      style="color: white; font-size:20px; font-weight:700;"
                    >
                     신청 가입<br><v-divider style="margin:5px;"></v-divider><span style="font-size:12px; font-weight:400;">관리자의 가입승인이 있어야<br>활동이 가능합니다.</span>
                    </div>
                  </v-card>
                </v-item>
              </v-row>
